...
func _on_timer_timeout():
	...
	if spawner.can_spawn(tile, world):
		...
		if other_mob:
			var dist: int= other_mob.distance_to_tile(tile)
			if dist < mob_def.min_distance_to_other:
				return
			elif other_mob.type == mob_def and dist < mob_def.min_distance_to_same:
				return
			
			other_mob= get_world().get_closest_mob(tile, mob_def)
			if dist < mob_def.min_distance_to_same:
				return
